...
namespace exqudens {
  ...
  void Sockets::setLogFunction(const std::function<void(const std::string&, const unsigned short&, const std::string&, const std::string&, const size_t&)>& value) {
    logFunction = value;
  }
  ...
  std::shared_ptr<SocketInterface> Sockets::createServer() {
    try {
      if (!initialized) {
        std::string errorMessage = "Not initialized. Call init(...) before usage";
        throw std::runtime_error(CALL_INFO + ": " + errorMessage);
      }
      std::shared_ptr<SocketInterface> result = nullptr;
  ...
  std::shared_ptr<SocketInterface> Sockets::createClient() {
    try {
      if (!initialized) {
        std::string errorMessage = "Not initialized. Call init(...) before usage";
        throw std::runtime_error(CALL_INFO + ": " + errorMessage);
  ...
  void Sockets::log(const std::string& message, const unsigned short& level, const std::string& function, const std::string& file, const size_t& line) {
    try {
      if (logFunction) {
